...
class execRunner(object):
    ...
    def __initializeData(self):
        ...
        # Options = namedtuple('Options', ['connection', 'module_path', 'forks', 'become', 'remote_user', 'become_method', 'become_user', 'check'])
        # initialize needed objects
        self.variable_manager = VariableManager()
        self.loader = DataLoader()
        self.options = Options(connection='smart', module_path='/data/trustops/ENV/bin', forks=100, timeout=15,
                               remote_user='root', ask_pass=False, private_key_file=None, ssh_common_args=None,
                               ssh_extra_args=None,
                               sftp_extra_args=None, scp_extra_args=None, become=None, become_method=None, become_user=None, ask_value_pass=False, verbosity=None, check=False, listhosts=False,
                               listtasks=False, listtags=False, syntax=False)
        # self.options = Options(connection='smart', module_path='/data/trustops/ENV/bin/ansible', forks=100, become=True, remote_user='root', become_method='sudo', become_user='root', check=False)
...
if __name__=='__main__':
    # _inv=MyInventory()
    # _resource={
    #             "NTS-CFB-BI-JTC-WEB-MOP": {
    #                 "hosts": [{"SHB-L0065457": "30.4.30.53", "port": "22", "username": "root", "password": "dT3mctzpt"}],
    #                 "vars": {"var1": '', "var2": ''}
    #             }
    #         }
    #{"hostname": IP, "port": port, "username": username, "password": password}
    _resource = [{"hostname": '30.4.30.53', "port": '22', "username": 'root', "password": 'dT3mctzpt'}]
    engine=execRunner(_resource)
    engine.run("30.4.30.53",'shell', 'ls')
